...
;(function (Zepto) {
    ...
    var Base = {
        ...
        getEnv : function(){
            ...
            else if( host.search('test2') != -1 || host.search('10.1.1.55') != -1 ) {
                return 'test2';
            }
...
;(function () {
    ...
    mutual.isAppInside = function(){
        return window.navigator.userAgent.indexOf("leadbank|APP");
    }
    ...
    mutual.callAppPhone = function(phone){
        var title = '';
        switch(true){
            case (phone=='400-858-0298'):
                title = '产品咨询'
                break;
            case (phone=='400-921-7733'):
                title = '客服电话'
                break;
            case (LeadBase.reg.phone.test(phone)):
                title = '理财师电话'
                break;
            default:
                title = '客服电话'
        ...
        mutual.jsCallNative({
            'nativeName': 'common.system.call',
            'parameter': {'phone': phone || '400-921-7733','title': title}
        })
    }
    //type 1 风险测评
    //type 2 合格投资者认证
    //type 3 通知高尔夫高端权益报名成功
    mutual.callAppStutas = function(type){
        mutual.jsCallNative({
            'nativeName': 'common.h5.notification',
            'parameter': {
                'type': type,
                'userInfo':{
                    'result': '0'
    ...
    //call reservation
    mutual.callReservation = function(data){
        openURL('leadwealth://page.lw/product.reservation?code='+data.goodsCode+'&name='+data.productName+'&type='+data.functionType);
    }

    mutual.saveImage = function(imageURL){
        mutual.jsCallNative({
            'nativeName': 'common.photo.save',
            'parameter': {
                'imageURL': imageURL
            }
        })
    ...
    //open pdf
    mutual.openPdfFile = function(url,name){
        if(phoneType.match(/android/i)){
            openURL('leadbank://page.ld/common.pdf?url='+url+'&name='+name);
        }else{
            mutual.setFixTitles(name);
